...
对于需要频繁读取但是不需要频繁写的共享数据，RCU是一种非常成功的同步机制。它可以实现多线程无阻塞地读取数据，就算有线程在修改数据时读者也不会被阻塞，且读取数据时除了内存屏障外没有额外同步开销，不会读取到不完整的数据，尽管数据可能不是最新的。
...
FTL OS的fork性能与预测性能相差甚远。经过反复检查代码后发现，RCU的回收过程发生在了线程环境下。这会导致什么问题？这意味着在释放RCU资源时这个线程也被占有了，无法被其他CPU获取到，显著增加了延迟。RCU的释放应该在调度器之外进行，即使发生了RCU回收，原来的线程也可以由另一个CPU运行。

但是进行这个优化后性能还是不稳定，在一次测试中，FTL OS关闭了帧内存的RCU释放，改为了立刻释放回帧分配器。这次测试中FTL OS的页表相关操作的速度提高了3倍以上！这可能是因为帧页面都是4KB对齐导致的。FTL OS的帧分配器的数据结构是侵入式链表，侵入式链表就放置在页面的头部。这会带来什么问题？首先，所有帧都是4KB对齐的，每个帧都会占有一个独立的TLB缓存，因此大规模的帧操作会让TLB整体无效。第二，U74核心使用32KB 8-way的数据L1 cache。4KB对齐的侵入式链表节点会全部命中于数据cache中的同一个组中，导致cache根本没法发挥作用，陷入了频繁的内存读取操作。如果我们立刻释放帧，那么在释放时帧依然存在于cache中，减少了一次缓慢的内存访问。

3倍的性能真实太让人心动了，但内存的安全性还是很重要。有什么办法在提高性能时保证内存的安全性呢？首先，对于单线程的进程，不会出现这种情况，因为这个页表被独占了，不存在其他其他CPU的并行访问，而当其他CPU运行本进程之前一定会从多核通信信箱中获取到刷表请求，因此我们只需要保证多线程情况下的安全性。解决方法非常简单，我们可以先将待释放的页面暂存，然后调用sbi提供的`remote_sfence_vma`函数击落其他CPU的TLB。

`remote_sfence_vma`的实现方式在M态由SBI代理，非常缓慢，在qemu上耗时达到了恐怖的50us，且发送的CPU越多越慢。我们应该将刷新的核心数降低到最小，只对必要的CPU发送，甚至不发送。怎么保证这一点呢？我们可以先将刷表申请发送给每个CPU，再读取每个CPU正在运行的pid，和本CPU相同则刷新。读取pid的时间地必须在发送刷表申请之后，因为如果在刷表之前读取，其他CPU可能在这个时间段内趁虚而入，既不会被`remote_sfence_vma`击落也不会被多核通信信箱刷表。改变顺序后就能保证这一点，因为如果读取到的pid是相同pid，`remote_sfence_vma`可以击落它的TLB，如果是不同的pid，它在下次运行本进程之前一定会通过信箱获取到刷表要求。当然，另一个CPU的pid必须在处理信箱之前设置。

增加页面权限不需要主动通知其他CPU，因为其他CPU完全可以主动引发页错误，这时再刷表不会有任何安全问题。但是减小权限，例如可写变为只读、取消映射等，必须使用上述方式刷表。

目前lmbench不存在多线程测试程序，因此直接释放帧不会导致内存安全问题，上述安全性修改会延后进行。
