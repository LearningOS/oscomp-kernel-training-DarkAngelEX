...
当考虑超时机制后情况发生了变化，因为此时睡眠者不再由唤醒者唤醒了，而是由自身唤醒。由自身唤醒的数据结构需要修改等待队列将自身撤销，因此睡眠者需要储存队列锁的位置。
...
如果存在，就可以直接在此阶段将等待器的队列指针从A改为B。但这需要处理死锁的可能，最简单的方式是规定某种全局性顺序并强制两个锁按顺序获取，最简单的顺序就是内存的地址。
