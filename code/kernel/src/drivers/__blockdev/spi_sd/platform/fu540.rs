...
impl SPIImpl {
    ...
    fn rx_deque(&self) -> u8 {
        ...
        let mut result: u32 = self.spi.rxdata.read();
        while (result & (1u32 << 31)) != 0 {
            // println!("[kernel] spi warning: attempting to read empty fifo");
            result = self.spi.rxdata.read();
        }
        result as u8
    }
...
impl SPIActions for SPIImpl {
    ...
    fn switch_cs(&self, enable: bool, csid: u32) {
        ...
        // #[cfg(feature = "qemu_sd")]
        // let mode = Mode::OFF;
        // #[cfg(feature = "sifive_sd")]
        let mode = Mode::AUTO;
        self.spi
            .csmode
            .switch_csmode(if enable { Mode::HOLD } else { mode });
    ...
    fn recv_data(&self, chip_select: u32, rx_buf: &mut [u8]) {
        ...
        while remaining != 0usize {
            ...
            let n_words = core::cmp::min(8, remaining);
