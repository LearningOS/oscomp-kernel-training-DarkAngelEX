...
impl<T: SPIActions> SDCard<T> {
    ...
    fn retry_cmd(
        ...
        crc: u8,
        retry_times: u32,
        success_cond: &dyn Fn(u8) -> bool,
    ) -> Result<u8, u8> {
        let mut resp = self.get_response();

        for i in 0..retry_times {
            //   log!("sd":"retry_cmd">"{:?} retried {} time", cmd, i);
        ...
        // log!("sd":"retry_cmd">"{:?} responded with code {:#x}", cmd, resp);
        return Err(resp);
    }
    ...
    pub fn init(&mut self) -> Result<SDCardInfo, InitError> {
        ...
        if let Err(resp) = self.retry_cmd(CMD::CMD0, 0, 0x95, 200, &|x| -> bool { x == 0x01 }) {
            return Err(InitError::CMDFailed(CMD::CMD0, 0));
        }

        // log!("sd":"init">"send cmd8");
        result = match self.retry_cmd(CMD::CMD8, 0x01AA, 0x87, 2, &|x| -> bool {
            x == 0x01 || x == 0x05
        }) {
            Ok(resp) => resp,
            Err(resp) => {
                return Err(InitError::CMDFailed(CMD::CMD8, resp));
            }
        };
        ...
        if result != 0x01 {
            ...
            while result != 0x00 {
                ...
                self.end_cmd();
                // log!("sd":"init">"send acmd41");
                self.send_cmd(CMD::ACMD41, 0x40000000, 0);
                // log!("sd":"init">"get_response acmd41");
                result = self.get_response();
                // log!("sd":"init">"end acmd41");
                self.end_cmd();
            }
        } else {
            ...
            for i in 0..4 {
                // log!("sd":"init">"OCR[{}]: 0x{:x}", i, frame[i]);
            }
    ...
    pub fn read_sector(&self, data_buf: &mut [u8], sector: u32) -> Result<(), ()> {
        ...
        for chunk in data_buf.chunks_mut(SEC_LEN) {
            //   log!("sd":"read">"reading sector {}", cur_sector);
            ...
            loop {
                ...
                if count == 0 {
                    //   log!("sd":"read">"cmd 17 returned error code {:#x}", res);
                    self.end_cmd();
                    return Err(());
                }
            ...
            let mut resp = self.get_response();
            if resp != SD_START_DATA_SINGLE_BLOCK_READ {
                // log!("sd":"read">"sd responding {:#x} rather than 0xfe", resp);
                resp = self.get_response();
                break;
            }
            ...
            cur_sector += match self.is_hc {
                false => SEC_LEN as u32,
                _ => 1,
            };
        }
    ...
    pub fn write_sector(&self, data_buf: &[u8], sector: u32) -> Result<(), ()> {
        ...
        let frame = [0xff, SD_START_DATA_SINGLE_BLOCK_WRITE];
        ...
        for trunk in data_buf.chunks(SEC_LEN) {
            ...
            loop {
                ...
                if count == 0 {
                    //   log!("sd":"read">"cmd 24 returned error code {:#x}", res);
                    self.end_cmd();
                    return Err(());
                }

                if res == 0x0 {
                    //   log!("test_cannot_run_mmap_munmap":"write_sec">"cmd24 resp: {}", res);
                    break;
                }
            ...
            let mut resp = self.get_response() & 0x1F;
            if resp != 0x5 {
                // log!("test_cannot_run_mmap_munmap":"write_sec">"sec: {:#x}, resp: {}", cur_sector, resp);
            }
