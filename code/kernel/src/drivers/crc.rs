//! SD文档说连续512个0xff的块的CRC16是0x7FA1
//!
//! 但是CRC16有一大堆变种, 应该用哪一个? 在这个网站里敲了512个ff:
//! https://www.23bei.com/tool/60.html
//!
//! 测试SD的CRC校验为CRC16-XMODEM

pub fn crc7_be_byte(crc: u8, data: u8) -> u8 {
    CRC7_BE_SYNDROME_TABLE[(crc ^ data) as usize]
}

pub fn crc7_be(crc: u8, buffer: &[u8]) -> u8 {
    buffer.iter().copied().fold(crc, crc7_be_byte)
}

pub fn crc16_xmodem_byte(crc: u16, data: u8) -> u16 {
    (crc << 8) ^ CRC16_XMODEM_TABLE[((crc >> 8) as u8 ^ data) as usize]
}

pub fn crc16_xmodem(crc: u16, buffer: &[u8]) -> u16 {
    buffer.iter().copied().fold(crc, crc16_xmodem_byte)
}

const CRC7_BE_SYNDROME_TABLE: [u8; 256] = [
