...
pub async fn open_file(path: &[&str], flags: OpenFlags, _mode: Mode) -> SysR<Arc<dyn VfsInode>> {
    stack_trace!();
...
pub async fn unlink<'a>(path: &[&str], _flags: OpenFlags) -> SysR<()> {
    stack_trace!();
    delete_any(path).await
}

async fn create_any(path: &[&str], is_dir: bool, read_only: bool) -> SysR<()> {
...
async fn delete_file(path: &[&str]) -> SysR<()> {
    let (name, dir) = split_search_path(path).await?;
    dir.delete_file(name).await
}
async fn delete_dir(path: &[&str]) -> SysR<()> {
    let (name, dir) = split_search_path(path).await?;
    dir.delete_dir(name).await
}
async fn delete_any(path: &[&str]) -> SysR<()> {
    let (name, dir) = split_search_path(path).await?;
    dir.delete_any(name).await
}

async fn search_any(path: &[&str]) -> SysR<Arc<ShmInode>> {
...
async fn split_search_path<'a>(path: &[&'a str]) -> SysR<(&'a str, Arc<ShmInode>)> {
...
async fn search_dir(mut path: &[&str]) -> SysR<Arc<ShmInode>> {
